...
import DashboardNotification from '../dashboard/components/dashboardNotification/DashboardNotification';
import { useHotkeys } from 'react-hotkeys-hook';
import { useNavigate } from 'react-router-dom';
import { webAppContext } from '../contexts/contexts';
import SidebarInvestor from './components/Sidebar/SidebarInvestor';
...
export default function PrivateDashboard() {
    ...
    const setUserKeyboardShortcuts = async () => {
        await axios.post('/api/investor-settings/', {}, {
            ...
            if("firstName" in result[0]){
                settings_update = { ...settings_update, userProfile: { ...settings_update.userProfile, firstName: result[0].firstName} }
                // __updateWebAppSettings({ ...__webAppSettings, activeThemeName: result[0].theme })
            }
            ...
            console.log(settings_update, "setting upf")
    ...
    useHotkeys(__webAppSettings.keyboardShortcuts.pVIWallet, (event) => {
        event.preventDefault();
        navigate('/investor/wallet');
    });

    useHotkeys(__webAppSettings.keyboardShortcuts.pVIInvestment, (event) => {
        event.preventDefault();
        navigate('/investor/investments');
    });

    useHotkeys(__webAppSettings.keyboardShortcuts.pVIIndInt, (event) => {
        event.preventDefault();
        navigate('/investor/interest');
    });

    useHotkeys(__webAppSettings.keyboardShortcuts.pVITrans, (event) => {
        event.preventDefault();
        navigate('/investor/transactions');
    });

    useHotkeys(__webAppSettings.keyboardShortcuts.pVIAutoInv, (event) => {
        event.preventDefault();
        navigate('/investor/auto-investment');
    });
